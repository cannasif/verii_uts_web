...
export function UtsTuketiciVermeListPage() {
  ...
  const query = useQuery({
    ...
    queryFn: getAllUtsTuketiciVermeList,
  });

  const columns = useMemo<DataGridColumn<UtsTuketiciVermeListItem>[]>(() => [
  ...
  return (
    <div className="space-y-6">
      ...
      <UtsViewGrid
        pageKey="uts-tuketici-verme-list-grid"
        userId={user?.id}
        rows={query.data?.data ?? []}
        columns={columns}
        rowKey={(row) => `${row.chk}-${row.siraNo ?? 0}-${row.bno ?? 'bos'}-${row.lsNo ?? 'bos'}`}
        exportFileName="uts-tuketici-verme-list"
