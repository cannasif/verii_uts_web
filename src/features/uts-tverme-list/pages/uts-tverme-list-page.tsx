...
import { getAllUtsTVermeList, type UtsTVermeListItem } from '@/features/uts-tverme-list/api/uts-tverme-list-api';
import { useAuthStore } from '@/stores/auth-store';
import { useUiStore } from '@/stores/ui-store';

export function UtsTVermeListPage() {
  const { t } = useTranslation(['uts-tverme-list-management', 'common']);
  const user = useAuthStore((state) => state.user);
  const theme = useUiStore((state) => state.theme);
  const isLight = theme === 'light';

  const query = useQuery({
    queryKey: ['uts-tverme-list'],
    queryFn: getAllUtsTVermeList,
  });

  const columns = useMemo<DataGridColumn<UtsTVermeListItem>[]>(() => [
    ...
    { key: 'vkn', label: t('vkn'), sortable: true },
    ...
    createDataGridEditActionsColumn<UtsTVermeListItem>({
